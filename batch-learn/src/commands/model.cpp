...
inline float Mean_Weigted_LogLoss(float p, float y){
    float yp = min(max(p,10e-7f),1-10e-7f);
    float loss_fn=y*log(yp);
    float loss_fp=(1-y)*log(1-yp);
    return -(5*loss_fn+loss_fp);
}
    ...
    for (uint64_t bi = 0; bi < batches.size(); ++ bi) {
        ...
        for (auto mb = mini_batches.begin(); mb != mini_batches.end(); ++ mb) {
            for (auto ei = mb->first; ei < mb->second; ++ ei) {
                ...
                m.update(batch_features_data + start_offset, batch_features_data + end_offset, norm, -y * expnyt / (1+expnyt) * (y>.0?5.0:1.0));

                //loss += log(1+exp(-y*t));
                loss += Mean_Weigted_LogLoss(1/(1+exp(-t)), y>0?1.0f:0.0f);
            }
        }
